...
import { fileManagerAskSamQuestions, fileManagerAskSamAnswer } from "@/data/askSamFileManager";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function FileManager() {
  const { showSuccess, showAlert, showError, showInfo, addToast } = useToastContext();
  ...
  const handleItemShare = (id: string) => {
    const item = files.find(f => f.id === id);
    showInfo(`Share link for "${item?.name}" has been copied to clipboard.`);
  };
      ...
      <header className="bg-white px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex space-x-4">
            <AskSamPopup
              questions={fileManagerAskSamQuestions}
              onGenerateAnswer={fileManagerAskSamAnswer}
            />
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="sm" className="gap-2 bg-white text-black border border-gray-300 hover:bg-gray-100 !hover:text-foreground">
                  <Upload className="h-4 w-4" />
                  Import
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); showInfo('Import from Google Meeting'); }}>
                  Google Meeting
                </DropdownMenuItem>
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); showInfo('Import from Teams Meeting'); }}>
                  Teams Meeting
                </DropdownMenuItem>
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); showInfo('Import from Zoom Meeting'); }}>
                  Zoom Meeting
                </DropdownMenuItem>
                <DropdownMenuItem onClick={(e) => { e.preventDefault(); showInfo('Import from Hubspot Call'); }}>
                  Hubspot Call
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <FileManagerHeader
            ...
            onUploadFile={() => showInfo("File upload would be implemented here.")}
            onCreateFolder={() => handleCreateFolder()}
